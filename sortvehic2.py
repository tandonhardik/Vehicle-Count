...
in_type_counts = defaultdict(int)
out_type_counts = defaultdict(int)
...
while True:
    ...
    for box in results.boxes:
        ...
        detections.append([x1.item(), y1.item(), x2.item(), y2.item(), conf.item(), cls])
    ...
    for track in tracks:
        ...
        if track_id not in track_cy_history:
            track_cy_history[track_id] = cy
            continue
        ...
        if track_id not in counted_ids:
            if prev_cy < line_y and cy >= line_y:
                ...
                for det in detections:
                    if abs(cx - (det[0] + det[2]) / 2) < 30 and abs(cy - (det[1] + det[3]) / 2) < 30:
                        cls_id = det[5]
                        if cls_id in vehicle_classes:
                            in_type_counts[vehicle_classes[cls_id]] += 1
                        break
            ...
            for det in detections:
                if abs(cx - (det[0] + det[2]) / 2) < 30 and abs(cy - (det[1] + det[3]) / 2) < 30:
                    cls_id = det[5]
                    if cls_id in vehicle_classes:
                        out_type_counts[vehicle_classes[cls_id]] += 1
                    break
        
        track_cy_history[track_id] = cy
    
    # Display counts
    cv2.putText(frame, f'IN: {in_count}', (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
    cv2.putText(frame, f'OUT: {out_count}', (frame_width - 200, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
    
    # Display total counts
    cv2.putText(frame, f'IN: {in_count}', (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
    cv2.putText(frame, f'OUT: {out_count}', (frame_width - 200, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)

    # Show detailed vehicle type counts
    start_y = 80
    for i, (veh, cnt) in enumerate(in_type_counts.items()):
        cv2.putText(frame, f'{veh.capitalize()}: {cnt}', (20, start_y + i * 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

    for i, (veh, cnt) in enumerate(out_type_counts.items()):
        cv2.putText(frame, f'{veh.capitalize()}: {cnt}', (frame_width - 220, start_y + i * 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
